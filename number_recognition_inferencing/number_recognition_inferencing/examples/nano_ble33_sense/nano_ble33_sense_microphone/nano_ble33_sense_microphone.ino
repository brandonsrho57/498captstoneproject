...
/* Includes ---------------------------------------------------------------- */
#include <PDM.h>
#include <number_recognition_inferencing.h>
...
const char* VoiceControlUuid = "19b10002-e8f2-537e-4f6c-d104768a0618";
...
void loop()
{
    ...
    if (count >= 100){
      ...
      for (size_t ix = 0; ix < EI_CLASSIFIER_LABEL_COUNT; ix++) {
          ...
          if (result.classification[ix].value > 0.91){
            if(ix == 0){
              voiceCommand = 4;
            }
            else if (ix == 1){
            ...
            Serial.println("Voice Command change to:");
...
void controlPeripheral(BLEDevice peripheral) {
  ...
  BLECharacteristic VoiceControlCharacteristic = peripheral.characteristic(VoiceControlUuid);
  ...
  while (peripheral.connected()) {
    ...
    if (oldVoiceCommand != voiceCommand) {  
      oldVoiceCommand = voiceCommand;
      Serial.print("* Writing value to voice command characteristic: ");
      Serial.println(voiceCommand);
      gestureCharacteristic.writeValue((byte)voiceCommand);
      Serial.println("* Writing value to voice command characteristic done!");
      Serial.println(" ");
    }
